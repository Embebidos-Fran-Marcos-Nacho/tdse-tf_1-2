...
void task_adc_init(void *parameters)
{
    ...
    uint8_t fixed_percent = (APP_DIMMER_FIXED_PERCENT > 100u) ? 100u : (uint8_t)APP_DIMMER_FIXED_PERCENT;

    TEST_LOG("  %s is running - %s\r\n", GET_NAME(task_adc_init), p_task_adc);

    shared_data->adc_raw = 0u;
    shared_data->adc_percent = fixed_percent;
    shared_data->fan_delay_us = (uint16_t)(APP_FAN_DIM_DELAY_MIN_US +
                                  (((uint32_t)fixed_percent * (APP_FAN_DIM_DELAY_MAX_US - APP_FAN_DIM_DELAY_MIN_US)) / 100u));
...
void task_adc_update(void *parameters)
{
    ...
    {
        uint8_t fixed_percent = (APP_DIMMER_FIXED_PERCENT > 100u) ? 100u : (uint8_t)APP_DIMMER_FIXED_PERCENT;
        uint16_t fixed_delay = (uint16_t)(APP_FAN_DIM_DELAY_MIN_US +
                                 (((uint32_t)fixed_percent * (APP_FAN_DIM_DELAY_MAX_US - APP_FAN_DIM_DELAY_MIN_US)) / 100u));

        shared_data->adc_percent = fixed_percent;
        shared_data->fan_delay_us = fixed_delay;
        shared_data->ev_pote_changed = false;
